...
from chatflow_ai.agent.message_processor import (
    MessageProcessor,
    ProcessorConfig,
    MessageResponse,
)
from chatflow_ai.agent.actions import register_action, Action
from chatflow_ai.agent.graph import (
...
from chatflow_ai.core.stores import create_tracker_store, TrackerStore
from chatflow_ai.dialogue_understanding.flow import FlowsList, FlowLoader
from chatflow_ai.dialogue_understanding.generator import LLMCommandGenerator
from chatflow_ai.dialogue_understanding.processor import CommandProcessor
from chatflow_ai.policies import PolicyEnsemble, FlowPolicy, EnterpriseSearchPolicy
from chatflow_ai.shared.yaml_loader import read_yaml_file
from chatflow_ai.shared.config import ChatFlowConfig, LLMConfig
...
class Agent:
    ...
    def load(
        ...
        """
        import tempfile
        from chatflow_ai.training.model_storage import (
        ...
        if project_path.is_file() and project_path.name.endswith(".tar.gz"):
            ...
            # 解压到临时目录
            temp_dir = tempfile.mkdtemp(prefix="chatflow_model_")
        ...
        if config_path.exists():
            ...
            if config_data:
                ...
                if "retrieval" in config_data:
                    from chatflow_ai.shared.config import RetrievalConfig
        ...
        if llm_config:
            from chatflow_ai.dialogue_understanding.generator import (
                LLMCommandGenerator,
                LLMGeneratorConfig,
            )
        ...
        if retriever_class_path:
            try:
                from chatflow_ai.retrieval import create_retriever
        ...
        if nlg_config and nlg_config.rephrase_enabled:
            try:
                from chatflow_ai.nlg import ResponseRephraser, RephraserConfig, TemplateNLG
