...
if TYPE_CHECKING:
    from chatflow_ai.core.tracker import DialogueStateTracker
    from chatflow_ai.core.domain import Domain
...
class ActionDefaultFallback(Action):
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        from chatflow_ai.dialogue_understanding.stack.stack_frame import CannotHandleStackFrame
...
class ActionChitChatResponse(Action):
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        from chatflow_ai.dialogue_understanding.stack.stack_frame import ChitChatStackFrame
...
class ActionHumanHandoff(Action):
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        from chatflow_ai.dialogue_understanding.stack.stack_frame import HumanHandoffStackFrame
...
class ActionTriggerSearch(Action):
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        from chatflow_ai.dialogue_understanding.stack.stack_frame import SearchStackFrame
...
class ActionCallOpenAPI(Action):
    """调用 OpenAPI 3.x 接口的通用动作。

    Flow step 通过 metadata.openapi 声明 spec、base_url、operation_id
    以及槽位到参数/请求体的映射。
    """

    @property
    def name(self) -> str:
        return "action_call_openapi"
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        variables = tracker.get_all_slots()
        timeout = float(openapi_config.get("timeout", 30.0))
        headers = resolve_mapping(openapi_config.get("headers", {}), variables)
        parameters = resolve_mapping(openapi_config.get("parameters", {}), variables)
        ...
        try:
            ...
            result.success = False
            result.add_response(f"接口调用配置错误：{e}")
            result.metadata["error"] = str(e)
            return result
        except Exception as e:
            result.success = False
            result.add_response(f"接口调用失败：{e}")
            result.metadata["error"] = str(e)
            return result

        response_config = openapi_config.get("response", {})
        ...
        success_message = response_config.get("success_message")
        failure_message = response_config.get("failure_message")
        message_variables = {
            **variables,
            **tracker.get_all_slots(),
            "status_code": api_response.get("status_code"),
        }

        if api_response.get("ok"):
            if success_message:
                result.add_response(render_template(success_message, message_variables))
        else:
            result.success = False
            if failure_message:
                result.add_response(render_template(failure_message, message_variables))
            else:
                result.add_response(f"接口调用失败，状态码：{api_response.get('status_code')}")
...
class ActionFlowCompleted(Action):
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        from chatflow_ai.dialogue_understanding.stack.stack_frame import CompletedStackFrame
...
_BUILTIN_ACTIONS: Dict[str, type] = {
    ...
    "action_call_openapi": ActionCallOpenAPI,
}
