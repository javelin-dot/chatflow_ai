...
def train_command(
    ...
    示例:
        chatflow train
        chatflow train --data ./my_data --output ./my_models
        chatflow train --dry-run  # 仅验证
    """
    verbose = ctx.obj.get("verbose", False)
    debug = ctx.obj.get("debug", False)
    
    click.echo("=" * 50)
    click.echo("ChatFlow AI - 模型训练")
    click.echo("=" * 50)
    ...
    try:
        # 导入训练模块
        from chatflow_ai.training.trainer import Trainer, TrainerConfig
        from chatflow_ai.shared.config import ChatFlowConfig
        
        # 加载配置（用于验证）
        click.echo("加载配置...")
        chatflow_config = ChatFlowConfig.load(str(config_path))
        click.echo("  配置加载成功")
        ...
        if dry_run:
            ...
            from chatflow_ai.dialogue_understanding.flow import FlowLoader
